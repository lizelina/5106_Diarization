...
class SpeakerDiarizationPipeline:
    ...
    def _check_directories(self):
        ...
        if not self.transcripts_aligned_dir.exists():
            raise FileNotFoundError(f"Required directory {self.transcripts_aligned_dir} not found. Please ensure it exists.")
    ...
    def diarize_file(self, file_path: str):
        ...
        diarization_model = Pipeline.from_pretrained(self.model_name, use_auth_token=self.use_auth_token).to(self.device)
    ...
    def assign_transcripts_to_speakers(self):
        ...
        for filename in filenames:
            ...
            res = self.assign_word_speakers(diarize_df, transcript_segments)

            merged_transcript_path = self.merged_transcript_dir / csv_name
            pd.DataFrame(res).to_csv(merged_transcript_path, index=False)

    def assign_word_speakers(self, diarize_df, transcript_segments, fill_nearest=False):
        """将发言者分配给转录文件中的片段"""
