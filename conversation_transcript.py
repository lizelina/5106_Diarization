import pandas as pd
import numpy as np
text_df = pd.read_csv('6Minute_short.csv')
speaker_df = pd.read_csv('cog_transcript_diarization.csv')

# Conversation-transcriptor merge function
...
def combine_whisper_and_pyannote2(text_df, speaker_df):
    # find overlapping speakers for each text segment
    overlap_list = []
    ...
    for idx, this_row in speaker_df.iterrows():
        ...
        xx_inds = ~((text_df['end'] < this_start) | (text_df['start'] > this_end))
        this_overlap_texts = text_df.loc[xx_inds, :].copy()  # Ensure a copy is created

        # Use .loc to avoid SettingWithCopyWarning
        this_overlap_texts.loc[:, 'speaker_start'] = this_start
        this_overlap_texts.loc[:, 'speaker_end'] = this_end
        this_overlap_texts.loc[:, 'speaker'] = this_speaker
...
res = combine_whisper_and_pyannote(text_df, speaker_df)
